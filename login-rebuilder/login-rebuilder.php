...
/*
 Copyright (C) 2013-2021 tmatsuur (Email: takenori dot matsuura at 12net dot jp)
...
define( 'LOGIN_REBUILDER_DB_VERSION', '2.7.2' );
...
?></div>
<div id="primary-log" style="<?php esc_attr_e( self::LOG_BOX_STYLES ); ?>">
<?php
...
?></div>
<div id="rest-log" style="<?php esc_attr_e( self::LOG_BOX_STYLES  ); ?>">
<?php
...
<?php
	...
	private function _notify_admin_login( $user = null ) {
		...
		if ( isset( $user->user_email ) && isset( $user->user_login ) ) {
			...
			$message .= sprintf( __( 'User agent: %s' ), $this->user_agent ) . "\r\n\r\n";
			$message .= __( "If you are not logged in, promptly take appropriate measures." )."\r\n";
			$title = sprintf( __( '[%s] %s logged in.' ), $blogname, $login );
	...
	private function _notify_unknown_ip( $remote_addr = null ) {
		...
		if ( 0 < count( $dataset ) ) {
			$message = __( "IP address information:" )."\r\n\r\n";
			foreach ( $dataset as $key => $data ) {
				if ( 0 < $key ) {
					$message .= "------------------------------\r\n";
				}
				$message .= sprintf( __( 'IP address: %s' ), $data->ip ) . "\r\n";
				$message .= sprintf( __( 'Count: %d' ), $data->count ) . "\r\n";
				$message .= sprintf( __( 'Date time: %s' ), date_i18n( 'Y-m-d H:i:s', strtotime( $data->time ) ) ) . "\r\n";
				$message .= sprintf( __( 'User agent: %s' ), $data->ua ) . "\r\n\r\n";
			}
			$message .= __( "In case of a suspicious IP address, please take appropriate measures immediately." )."\r\n";

			$to = sanitize_email( trim( get_option( 'admin_email' ) ) );
			$blogname = wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
			$title = sprintf( __( '[%s] Unknown IP address.' ), $blogname );
			wp_mail( $to, $title, $message );
		}
	}
