	...
	class SPLC_FREE_Field_spacing extends SPLC_FREE_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['all'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[all]' ) ) . '" value="' . esc_attr( $value['all'] ) . '"' . $placeholder . ' class="splogocarousel-input-number' . esc_attr( $is_unit ) . '" step="any" />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[' . $property . ']' ) ) . '" value="' . esc_attr( $value[ $property ] ) . '"' . $placeholder . ' class="splogocarousel-input-number' . esc_attr( $is_unit ) . '" step="any" />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo ( $unit ) ? '<span class="splogocarousel--label splogocarousel--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
				}
			}
