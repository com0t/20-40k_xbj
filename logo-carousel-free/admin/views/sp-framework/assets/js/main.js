; (function ($, window, document, undefined) {
  ...
  //     var $nav      = $(this),
  //         $links    = $nav.find('a'),
  //         $sections = $nav.parent().find('.splogocarousel-section'),
  //         $last;
  ...
  // Field: accordion
  //
  $.fn.splogocarousel_field_accordion = function () {
    return this.each(function () {

      var $titles = $(this).find('.splogocarousel-accordion-title');

      $titles.on('click', function () {

        var $title = $(this),
          $icon = $title.find('.splogocarousel-accordion-icon'),
          $content = $title.next();
        ...
        if (!$content.data('opened')) {
          ...
          $content.splogocarousel_reload_script();
          $content.data('opened', true);

        }

        $content.toggleClass('splogocarousel-accordion-open');
  ...
  // Field: backup
  //
  $.fn.splogocarousel_field_backup = function () {
    return this.each(function () {
      ...
      var base = this,
        $this = $(this),
        $body = $('body'),
        $import = $this.find('.splogocarousel-import'),
        $reset = $this.find('.splogocarousel-reset');
      ...
      base.notificationOverlay = function () {
        ...
        if (wp.customize.notifications && wp.customize.OverlayNotification) {
          ...
          wp.customize.notifications.add(new wp.customize.OverlayNotification('splogocarousel_field_backup_notification', {
            type: 'default',
            message: '&nbsp;',
            loading: true
          }));
      ...
      $reset.on('click', function (e) {
        ...
        if (SPLC.vars.is_confirm) {

          base.notificationOverlay();

          window.wp.ajax.post('splogocarousel-reset', {
            ...
            .fail(function (response) {
              ...
              wp.customize.notifications.remove('splogocarousel_field_backup_notification');
            });
      ...
      $import.on('click', function (e) {
        ...
        if (SPLC.vars.is_confirm) {
          ...
          window.wp.ajax.post('splogocarousel-import', {
            unique: $import.data('unique'),
            nonce: $import.data('nonce'),
            data: $this.find('.splogocarousel-import-data').val()
          }).done(function (response) {
            window.location.reload(true);
          }).fail(function (response) {
            alert(response.error);
  ...
  // Field: date
  //
  $.fn.splogocarousel_field_date = function () {
    return this.each(function () {

      var $this = $(this),
        $inputs = $this.find('input'),
        settings = $this.find('.splogocarousel-date-settings').data('settings'),
        wrapper = '<div class="splogocarousel-datepicker-wrapper"></div>',
        $datepicker;

      var defaults = {
        showAnim: '',
        beforeShow: function (input, inst) {
          $(inst.dpDiv).addClass('splogocarousel-datepicker-wrapper');
        },
        onClose: function (input, inst) {
          $(inst.dpDiv).removeClass('splogocarousel-datepicker-wrapper');
        },
      };
  ...
  // Field: gallery
  //
  $.fn.splogocarousel_field_gallery = function () {
    return this.each(function () {

      var $this = $(this),
        $edit = $this.find('.splogocarousel-edit-gallery'),
        $clear = $this.find('.splogocarousel-clear-gallery'),
        $list = $this.find('ul'),
        $input = $this.find('input'),
        $img = $this.find('img'),
        wp_media_frame;

      $this.on('click', '.splogocarousel-button, .splogocarousel-edit-gallery', function (e) {

        var $el = $(this),
          ids = $input.val(),
          what = ($el.hasClass('splogocarousel-edit-gallery')) ? 'edit' : 'add',
  ...
  // Field: group
  //
  $.fn.splogocarousel_field_group = function () {
    return this.each(function () {

      var $this = $(this),
        $fieldset = $this.children('.splogocarousel-fieldset'),
        $group = $fieldset.length ? $fieldset : $this,
        $wrapper = $group.children('.splogocarousel-cloneable-wrapper'),
        $hidden = $group.children('.splogocarousel-cloneable-hidden'),
        $max = $group.children('.splogocarousel-cloneable-max'),
        $min = $group.children('.splogocarousel-cloneable-min'),
      ...
      var update_title_numbers = function ($selector) {
        $selector.find('.splogocarousel-cloneable-title-number').each(function (index) {
          $(this).html(($(this).closest('.splogocarousel-cloneable-item').index() + 1) + '.');
        });
      };

      $wrapper.accordion({
        header: '> .splogocarousel-cloneable-item > .splogocarousel-cloneable-title',
        collapsible: true,
        active: false,
        animate: false,
        heightStyle: 'content',
        icons: {
          'header': 'splogocarousel-cloneable-header-icon fa fa-angle-right',
          'activeHeader': 'splogocarousel-cloneable-header-icon fa fa-angle-down'
        },
        activate: function (event, ui) {
          ...
          if ($panel.length && !$panel.data('opened')) {
            ...
            var $title = $header.find('.splogocarousel-cloneable-value');

            $first.on('change keyup', function (event) {
              $title.text($first.val());
            });

            $panel.splogocarousel_reload_script();
            $panel.data('opened', true);
            $panel.data('retry', false);

          } else if ($panel.data('retry')) {

            $panel.splogocarousel_reload_script_retry();
            $panel.data('retry', false);
      ...
      $wrapper.sortable({
        axis: 'y',
        handle: '.splogocarousel-cloneable-title,.splogocarousel-cloneable-sort',
        helper: 'original',
        cursor: 'move',
        ...
        start: function (event, ui) {
          ...
          ui.item.children('.splogocarousel-cloneable-content').data('retry', true);

        },
        update: function (event, ui) {

          SPLC.helper.name_nested_replace($wrapper.children('.splogocarousel-cloneable-item'), field_id);
          $wrapper.splogocarousel_customizer_refresh();
      ...
      $group.children('.splogocarousel-cloneable-add').on('click', function (e) {

        e.preventDefault();

        var count = $wrapper.children('.splogocarousel-cloneable-item').length;
        ...
        var $cloned_item = $hidden.splogocarousel_clone(true);

        $cloned_item.removeClass('splogocarousel-cloneable-hidden');
        ...
        $wrapper.splogocarousel_customizer_refresh();
        $wrapper.splogocarousel_customizer_listen({ closest: true });
      ...
      var event_clone = function (e) {
        ...
        var $this = $(this),
          ...
          $cloned_helper = $parent.children('.splogocarousel-cloneable-helper').splogocarousel_clone(true),
          $cloned_title = $parent.children('.splogocarousel-cloneable-title').splogocarousel_clone(),
          $cloned_content = $parent.children('.splogocarousel-cloneable-content').splogocarousel_clone(),
          $cloned_item = $('<div class="splogocarousel-cloneable-item" />');
      ...
      $wrapper.children('.splogocarousel-cloneable-item').children('.splogocarousel-cloneable-helper').on('click', '.splogocarousel-cloneable-clone', event_clone);
      $group.children('.splogocarousel-cloneable-hidden').children('.splogocarousel-cloneable-helper').on('click', '.splogocarousel-cloneable-clone', event_clone);
      ...
      var event_remove = function (e) {
        ...
        $(this).closest('.splogocarousel-cloneable-item').remove();
      ...
      $wrapper.children('.splogocarousel-cloneable-item').children('.splogocarousel-cloneable-helper').on('click', '.splogocarousel-cloneable-remove', event_remove);
      $group.children('.splogocarousel-cloneable-hidden').children('.splogocarousel-cloneable-helper').on('click', '.splogocarousel-cloneable-remove', event_remove);
  ...
  // Field: icon
  //
  $.fn.splogocarousel_field_icon = function () {
    return this.each(function () {

      var $this = $(this);

      $this.on('click', '.splogocarousel-icon-add', function (e) {

        e.preventDefault();

        var $button = $(this);
        var $modal = $('#splogocarousel-modal-icon');

        $modal.removeClass('hidden');

        SPLC.vars.$icon_target = $this;

        if (!SPLC.vars.icon_modal_loaded) {

          $modal.find('.splogocarousel-modal-loading').show();

          window.wp.ajax.post('splogocarousel-get-icons', {
            nonce: $button.data('nonce')
          }).done(function (response) {

            $modal.find('.splogocarousel-modal-loading').hide();

            SPLC.vars.icon_modal_loaded = true;

            var $load = $modal.find('.splogocarousel-modal-load').html(response.content);
            ...
            $load.on('click', 'i', function (e) {
              ...
              SPLC.vars.$icon_target.find('.splogocarousel-icon-preview i').removeAttr('class').addClass(icon);
              SPLC.vars.$icon_target.find('.splogocarousel-icon-preview').removeClass('hidden');
              SPLC.vars.$icon_target.find('.splogocarousel-icon-remove').removeClass('hidden');
              SPLC.vars.$icon_target.find('input').val(icon).trigger('change');

              $modal.addClass('hidden');

            });

            $modal.on('change keyup', '.splogocarousel-icon-search', function () {
            ...
            $modal.find('.splogocarousel-modal-loading').hide();
            $modal.find('.splogocarousel-modal-load').html(response.error);
      ...
      $this.on('click', '.splogocarousel-icon-remove', function (e) {
        e.preventDefault();
        $this.find('.splogocarousel-icon-preview').addClass('hidden');
  ...
  // Field: map
  //
  $.fn.splogocarousel_field_map = function () {
    return this.each(function () {

      if (typeof L === 'undefined') { return; }

      var $this = $(this),
        $map = $this.find('.splogocarousel--map-osm'),
        $search_input = $this.find('.splogocarousel--map-search input'),
        $latitude = $this.find('.splogocarousel--latitude'),
        $longitude = $this.find('.splogocarousel--longitude'),
        $zoom = $this.find('.splogocarousel--zoom'),
      ...
      if (!$search_input.length) {
        $search_input = $('[data-depend-id="' + $this.find('.splogocarousel--address-field').data('address-field') + '"]');
      }
      ...
      $search_input.autocomplete({
        ...
        create: function (event, ui) {
          $(this).autocomplete('widget').addClass('splogocarousel-map-ui-autocomplate');
        }
      });
  ...
  // Field: media
  //
  $.fn.splogocarousel_field_media = function () {
    return this.each(function () {

      var $this = $(this),
        $upload_button = $this.find('.splogocarousel--button'),
        $remove_button = $this.find('.splogocarousel--remove'),
        $library = $upload_button.data('library') && $upload_button.data('library').split(',') || '',
        $auto_attributes = ($this.hasClass('splogocarousel-assign-field-background')) ? $this.closest('.splogocarousel-field-background').find('.splogocarousel--auto-attributes') : false,
        wp_media_frame;
      ...
      $upload_button.on('click', function (e) {
        ...
        wp_media_frame.on('select', function () {
          ...
          $this.find('.splogocarousel--id').val(attributes.id);
          $this.find('.splogocarousel--width').val(attributes.width);
          $this.find('.splogocarousel--height').val(attributes.height);
          $this.find('.splogocarousel--alt').val(attributes.alt);
          $this.find('.splogocarousel--title').val(attributes.title);
          $this.find('.splogocarousel--description').val(attributes.description);
          ...
          if ($auto_attributes) {
            $auto_attributes.removeClass('splogocarousel--attributes-hidden');
          }

          $remove_button.removeClass('hidden');

          $this.find('.splogocarousel--preview').removeClass('hidden');
          $this.find('.splogocarousel--src').attr('src', thumbnail);
          $this.find('.splogocarousel--thumbnail').val(thumbnail);
          $this.find('.splogocarousel--url').val(attributes.url).trigger('change');
      ...
      $remove_button.on('click', function (e) {
        ...
        if ($auto_attributes) {
          $auto_attributes.addClass('splogocarousel--attributes-hidden');
        }

        $remove_button.addClass('hidden');
        $this.find('input').val('');
        $this.find('.splogocarousel--preview').addClass('hidden');
        $this.find('.splogocarousel--url').trigger('change');
  ...
  // Field: repeater
  //
  $.fn.splogocarousel_field_repeater = function () {
    return this.each(function () {

      var $this = $(this),
        $fieldset = $this.children('.splogocarousel-fieldset'),
        $repeater = $fieldset.length ? $fieldset : $this,
        $wrapper = $repeater.children('.splogocarousel-repeater-wrapper'),
        $hidden = $repeater.children('.splogocarousel-repeater-hidden'),
        $max = $repeater.children('.splogocarousel-repeater-max'),
        $min = $repeater.children('.splogocarousel-repeater-min'),
        field_id = $wrapper.data('field-id'),
        max = parseInt($wrapper.data('max')),
        min = parseInt($wrapper.data('min'));

      $wrapper.children('.splogocarousel-repeater-item').children('.splogocarousel-repeater-content').splogocarousel_reload_script();

      $wrapper.sortable({
        axis: 'y',
        handle: '.splogocarousel-repeater-sort',
        helper: 'original',
        cursor: 'move',
        placeholder: 'widget-placeholder',
        update: function (event, ui) {

          SPLC.helper.name_nested_replace($wrapper.children('.splogocarousel-repeater-item'), field_id);
          $wrapper.splogocarousel_customizer_refresh();
          ui.item.splogocarousel_reload_script_retry();

        }
      });

      $repeater.children('.splogocarousel-repeater-add').on('click', function (e) {

        e.preventDefault();

        var count = $wrapper.children('.splogocarousel-repeater-item').length;
        ...
        $cloned_item.removeClass('splogocarousel-repeater-hidden');
        ...
        $wrapper.append($cloned_item);
        $cloned_item.children('.splogocarousel-repeater-content').splogocarousel_reload_script();
        $wrapper.splogocarousel_customizer_refresh();
        $wrapper.splogocarousel_customizer_listen({ closest: true });
      ...
      var event_clone = function (e) {
        ...
        var $this = $(this),
          ...
          $cloned_content = $parent.children('.splogocarousel-repeater-content').splogocarousel_clone(),
          $cloned_helper = $parent.children('.splogocarousel-repeater-helper').splogocarousel_clone(true),
          $cloned_item = $('<div class="splogocarousel-repeater-item" />');
      ...
      $wrapper.children('.splogocarousel-repeater-item').children('.splogocarousel-repeater-helper').on('click', '.splogocarousel-repeater-clone', event_clone);
      $repeater.children('.splogocarousel-repeater-hidden').children('.splogocarousel-repeater-helper').on('click', '.splogocarousel-repeater-clone', event_clone);
      ...
      var event_remove = function (e) {
        ...
        $(this).closest('.splogocarousel-repeater-item').remove();
      ...
      $wrapper.children('.splogocarousel-repeater-item').children('.splogocarousel-repeater-helper').on('click', '.splogocarousel-repeater-remove', event_remove);
      $repeater.children('.splogocarousel-repeater-hidden').children('.splogocarousel-repeater-helper').on('click', '.splogocarousel-repeater-remove', event_remove);
  ...
  // Field: sortable
  //
  $.fn.splogocarousel_field_sortable = function () {
    return this.each(function () {

      var $sortable = $(this).find('.splogocarousel-sortable');
      ...
      $sortable.sortable({
        ...
        placeholder: 'widget-placeholder',
        update: function (event, ui) {
          $sortable.splogocarousel_customizer_refresh();
        }
      });

      $sortable.find('.splogocarousel-sortable-content').splogocarousel_reload_script();
  ...
  // Field: sorter
  //
  $.fn.splogocarousel_field_sorter = function () {
    return this.each(function () {

      var $this = $(this),
        $enabled = $this.find('.splogocarousel-enabled'),
        $has_disabled = $this.find('.splogocarousel-disabled'),
      ...
      $enabled.sortable({
        ...
        update: function (event, ui) {
          ...
          if (ui.item.parent().hasClass('splogocarousel-enabled')) {
            $el.attr('name', $el.attr('name').replace('disabled', 'enabled'));
          } else {
            $el.attr('name', $el.attr('name').replace('enabled', 'disabled'));
          }

          $this.splogocarousel_customizer_refresh();
  ...
  $.fn.splogocarousel_field_tabbed = function () {
    return this.each(function () {
      ...
      var $this = $(this),
        ...
        $contents = $this.find('.splogocarousel-tabbed-content');

      $contents.eq(0).splogocarousel_reload_script();
  ...
  // Field: upload
  //
  $.fn.splogocarousel_field_upload = function () {
    return this.each(function () {
  ...
  // Field: wp_editor
  //
  $.fn.splogocarousel_field_wp_editor = function () {
    return this.each(function () {

      if (typeof window.wp.editor === 'undefined' || typeof window.tinyMCEPreInit === 'undefined' || typeof window.tinyMCEPreInit.mceInit.splogocarousel_wp_editor === 'undefined') {
        return;
      }

      var $this = $(this),
        $editor = $this.find('.splogocarousel-wp-editor'),
        $textarea = $this.find('textarea');
      ...
      var uid = SPLC.helper.uid('splogocarousel-editor-');

      $textarea.attr('id', uid);

      // Get default editor settings
      var default_editor_settings = {
        tinymce: window.tinyMCEPreInit.mceInit.splogocarousel_wp_editor,
        quicktags: window.tinyMCEPreInit.qtInit.splogocarousel_wp_editor
      };
      ...
      if (field_editor_settings.tinymce === false) {
        ...
        $editor.addClass('splogocarousel-no-tinymce');
      }

      // Override editor quicktags settings
      if (field_editor_settings.quicktags === false) {
        default_editor_settings.quicktags = false;
        $editor.addClass('splogocarousel-no-quicktags');
      }
      ...
      // Add Media buttons
      if (field_editor_settings.media_buttons && window.splogocarousel_media_buttons) {

        var $editor_buttons = $editor.find('.wp-media-buttons');

        if ($editor_buttons.length) {

          $editor_buttons.find('.splogocarousel-shortcode-button').data('editor-id', uid);

        } else {

          var $media_buttons = $(window.splogocarousel_media_buttons);

          $media_buttons.find('.splogocarousel-shortcode-button').data('editor-id', uid);
  ...
  $.fn.splogocarousel_reload_script_retry = function () {
    return this.each(function () {
      ...
      if ($this.data('inited')) {
        $this.children('.splogocarousel-field-wp_editor').splogocarousel_field_wp_editor();
      }
  ...
  $.fn.splogocarousel_reload_script = function (options) {
    ...
    return this.each(function () {
      ...
      if (!$this.data('inited')) {
        ...
        // Field plugins
        $this.children('.splogocarousel-field-accordion').splogocarousel_field_accordion();
        $this.children('.splogocarousel-field-backup').splogocarousel_field_backup();
        $this.children('.splogocarousel-field-background').splogocarousel_field_background();
        $this.children('.splogocarousel-field-code_editor').splogocarousel_field_code_editor();
        $this.children('.splogocarousel-field-date').splogocarousel_field_date();
        $this.children('.splogocarousel-field-fieldset').splogocarousel_field_fieldset();
        $this.children('.splogocarousel-field-gallery').splogocarousel_field_gallery();
        $this.children('.splogocarousel-field-group').splogocarousel_field_group();
        $this.children('.splogocarousel-field-icon').splogocarousel_field_icon();
        $this.children('.splogocarousel-field-link').splogocarousel_field_link();
        $this.children('.splogocarousel-field-media').splogocarousel_field_media();
        $this.children('.splogocarousel-field-map').splogocarousel_field_map();
        $this.children('.splogocarousel-field-repeater').splogocarousel_field_repeater();
        $this.children('.splogocarousel-field-slider').splogocarousel_field_slider();
        $this.children('.splogocarousel-field-sortable').splogocarousel_field_sortable();
        $this.children('.splogocarousel-field-sorter').splogocarousel_field_sorter();
        ...
        $this.children('.splogocarousel-field-upload').splogocarousel_field_upload();
        $this.children('.splogocarousel-field-wp_editor').splogocarousel_field_wp_editor();

        // Field colors
        $this.children('.splogocarousel-field-border').find('.splogocarousel-color').splogocarousel_color();
        $this.children('.splogocarousel-field-background').find('.splogocarousel-color').splogocarousel_color();
        ...
        $this.children('.splogocarousel-field-link_color').find('.splogocarousel-color').splogocarousel_color();
        ...
        $this.children('.splogocarousel-field-image_select').find('.splogocarousel-siblings').splogocarousel_siblings();
        $this.children('.splogocarousel-field-layout_preset').find('.splogocarousel-siblings').splogocarousel_siblings();
        $this.children('.splogocarousel-field-palette').find('.splogocarousel-siblings').splogocarousel_siblings();
  ...
  $('.lc_input_shortcode').click(function (e) {
    e.preventDefault();
    /* Get the text field */
    var copyText = $(this);
  ...
  $('.lcp_export .splogocarousel--button').click(function (event) {
    event.preventDefault();
   ...
   // console.log($shortcode_ids);
    if ($shortcode_ids.length && $export_type === 'selected_shortcodes') {
      ...
      $('.splogocarousel-form-result.splogocarousel-form-success').text('No group selected.').show();
      setTimeout(function () {
  ...
  $('.lcp_import button.import').click(function (event) {
    event.preventDefault();
  ...
  $('.order_by_pro').find("select option[value='menu_order'],option[value='rand']").attr('disabled', 'disabled').addClass('pro_only');
  $('.order_by_pro').find("select option[value='zoom_in'],option[value='zoom_out']").attr('disabled', 'disabled').addClass('pro_only');
  $('.order_by_pro').find("select option[value='bottom'],option[value='top']").attr('disabled', 'disabled').addClass('pro_only');
  $('.order_by_pro').find("select option[value='always_gray'],option[value='gray_on_hover'],option[value='gray_with_normal']").attr('disabled', 'disabled').addClass('pro_only');
  $('.order_by_pro').find("select option[value='shadow_inset'],option[value='shadow_outset']").attr('disabled', 'disabled').addClass('pro_only');
  $('.order_by_pro').find("select option[value='top_center'],option[value='top_left'],option[value='bottom_left'],option[value='bottom_center'],option[value='bottom_right'],option[value='vertical_center'],option[value='vertical_center_inner'],option[value='vertical_center_inner_hover']").attr('disabled', 'disabled').addClass('pro_only');
