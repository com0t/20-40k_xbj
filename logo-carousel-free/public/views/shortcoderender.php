	...
	class SPLC_Shortcode_Render {
		...
		public static function splcp_html_show( $post_id, $logo_data, $main_section_title ) {
			...
			if ( 'true' === $dots || 'true' === $dots_mobile ) {
				...
				#logo-carousel-free-' . $post_id . '.sp-lc-container .sp-lc-pagination .swiper-pagination-bullet.swiper-pagination-bullet-active{background-color: ' . esc_attr( $dots_active_color ) . '; }
				';
			}
			...
			while ( $args->have_posts() ) :
				...
				$image                = has_post_thumbnail() && $show_image ? sprintf( '<img src="%1$s"%2$s alt="%3$s" width="%4$s" height="%5$s">', esc_url( $image_url[0] ), esc_html( $image_title_attr ), get_the_title(), esc_attr( $image_url[1] ), esc_attr( $image_url[2] ) ) : '';
